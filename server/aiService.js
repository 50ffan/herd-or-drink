...
async function generatePrompt(usedPrompts = []) {
  ...
  try {
    ...
    const msg = await getClient().messages.create({
      ...
      messages: [{
        role: 'user',
        content: `Du är spelledare för ett sällskapsspel på svenska. Ge mig EN rolig, öppen fråga på naturlig svenska för vuxna (PG-13). Frågan ska vara lätt att svara på men ge många olika svar. Variera tonen (rolig, lite spicy, eller vardaglig). Ge BARA frågan, inget annat.\n\nUndvik dessa frågor som redan använts:\n${used || '(inga)'}\n\nSkriv bara frågan:`
      }]
    });
...
async function groupAnswers(answers) {
  ...
  try {
    const answersText = answers
      .map(a => `- ${a.name}: "${a.text}" (svarade på ${a.responseTimeSeconds.toFixed(1)}s)`)
      .join('\n');
    const msg = await getClient().messages.create({
      model: 'claude-sonnet-4-6',
      max_tokens: 800,
      messages: [{
        role: 'user',
        content: `Du analyserar svar i ett sällskapsspel. Gruppera dessa svar efter likhet (ignorera stavfel, synonymer, singular/plural, versaler). Skriv en kort rolig roast (max 20 ord, på svenska, med spelarnamn).\n\nSvar:\n${answersText}\n\nSvara BARA med JSON (inga kodblock):\n{"groups":[{"label":"kortfattad etikett","members":[{"name":"spelarnamn","text":"deras svar","responseTime":1.2}]}],"herdLabel":"etiketten på den största gruppen eller null","roast":"rolig roast på svenska"}`
      }]
    });
